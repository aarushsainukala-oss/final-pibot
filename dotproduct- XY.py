    ...
    with mp_pose.Pose() as pose:
        while cap.isOpened():
            ...
            if all(is_visible(p) for p in [r_sh, r_el, r_wr, r_hp]):
                ...
                message = f"R_SHOULDER: {rs :.2f}"
                sock.sendto(message.encode() , (PI_IP , PORT))
                message = f"R_ELBOW: {re :.2f}"
                sock.sendto(message.encode() , (PI_IP , PORT))                
                #ser.write(f"{int(rs)}\n".encode())
            ...
            if all(is_visible(p) for p in [l_sh, l_el, l_wr, l_hp]):
                ...
                message = f"L_SHOULDER: {ls :.2f}"
                sock.sendto(message.encode() , (PI_IP , PORT))
                current_ls = ls
                #ser.write(f"{int(ls)}\n".encode())
                message = f"L_ELBOW: {180-le :.2f}"
                sock.sendto(message.encode() , (PI_IP , PORT))                
            ...
            # Show CV2 frame
            ser.write(f"{int(current_rs)} {int(current_ls)}\n".encode())
